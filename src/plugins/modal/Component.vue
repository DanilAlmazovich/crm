<template>
  <transition name="modal-fade">
    <div
      class="modal-container fixed h-screen w-full top-0 left-0 flex justify-center items-center pt-4"
      v-if="show"
      :class="confirmed ? 'z-40' : 'z-100'"
    >
      <slot name="menu"></slot>
      <div
        class="modal-block relative rounded-xl bg-white mb-8 py-5 px-6 max-h-full overflow-y-auto scrollbar"
      >
        <div class="flex justify-between items-center">
          ...
          >
            <svg
              class="fill-current text-red-400 w-3 h-3"
              x="0px"
              y="0px"
              viewBox="0 0 512 512"
      ...
      <div
        class="absolute h-full w-full z-back blackout top-0"
        v-if="show"
        @click="close"
      />
    </div>
